...
//need to write a code that says 'if this attribute youre looking for (included) exists, then run it, otherwise return 'not reported'.... otherwise the code freezes
...
// creating varibales from 'data' and 'included
    ...
    // if (marker.length < 1){
        for (i=0; i < data.length; i++){
            ...
            let popup = new mapboxgl.Popup().setText(
                // Remake once 'include' element updated
                ...
                `Bus Number: ${data[i].attributes.label} 
                Route: ${route} 
                Headed: ${transit} 
